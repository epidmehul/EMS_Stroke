...
high_lvo_parquet_list = sorted(args.input.glob('high_lvo_*.parquet'))
mid_lvo_parquet_list = sorted(args.input.glob('mid_lvo_*.parquet'))
low_lvo_parquet_list = sorted(args.input.glob('low_lvo_*.parquet'))
...
if __name__ == '__main__':
    with mp.Pool(num_cores) as pool:
        high_lvo_results = pool.starmap(group_all_data, list(zip(high_lvo_parquet_list, [False] * len(high_lvo_parquet_list))))
        high_lvo_psc_results = pool.starmap(group_all_data, list(zip(high_lvo_parquet_list, [True] * len(high_lvo_parquet_list))))

        mid_lvo_results = pool.starmap(group_all_data, list(zip(mid_lvo_parquet_list, [False] * len(mid_lvo_parquet_list))))
        mid_lvo_psc_results = pool.starmap(group_all_data, list(zip(mid_lvo_parquet_list, [True] * len(mid_lvo_parquet_list))))

        low_lvo_results = pool.starmap(group_all_data, list(zip(low_lvo_parquet_list, [False] * len(low_lvo_parquet_list))))
        low_lvo_psc_results = pool.starmap(group_all_data, list(zip(low_lvo_parquet_list, [True] * len(low_lvo_parquet_list))))
    grouped_avgs, intervals = zip(*high_lvo_results)
    psc_grouped_avgs, psc_intervals= zip(*high_lvo_psc_results)
    pd.concat(grouped_avgs).to_csv(args.output / 'high_lvo_all_cohort_avgs.csv')
    pd.concat(intervals).to_csv(args.output / 'high_lvo_map_scenario_intervals.csv', index = False)
    pd.concat(psc_grouped_avgs).to_csv(args.output / 'high_lvo_psc_cohort_avgs.csv')
    pd.concat(psc_intervals).to_csv(args.output / 'high_lvo_psc_map_scenario_intervals.csv', index = False)

    grouped_avgs, intervals = zip(*mid_lvo_results)
    psc_grouped_avgs, psc_intervals= zip(*mid_lvo_psc_results)
    pd.concat(grouped_avgs).to_csv(args.output / 'mid_lvo_all_cohort_avgs.csv')
    pd.concat(intervals).to_csv(args.output / 'mid_lvo_map_scenario_intervals.csv', index = False)
    pd.concat(psc_grouped_avgs).to_csv(args.output / 'mid_lvo_psc_cohort_avgs.csv')
    pd.concat(psc_intervals).to_csv(args.output / 'mid_lvo_psc_map_scenario_intervals.csv', index = False)

    grouped_avgs, intervals = zip(*low_lvo_results)
    psc_grouped_avgs, psc_intervals= zip(*low_lvo_psc_results)
    pd.concat(grouped_avgs).to_csv(args.output / 'low_lvo_all_cohort_avgs.csv')
    pd.concat(intervals).to_csv(args.output / 'low_lvo_map_scenario_intervals.csv', index = False)
    pd.concat(psc_grouped_avgs).to_csv(args.output / 'low_lvo_psc_cohort_avgs.csv')
    pd.concat(psc_intervals).to_csv(args.output / 'low_lvo_psc_map_scenario_intervals.csv', index = False)