...
def simulation(num_patients, patient_seed, map_seed, sens_spec_vals = np.array([[0.9, 0.6], [0.75, 0.75], [0.6, 0.9]]), thresholds = np.arange(0, 70, 10), config = None):
    ...
    # try:
    IVTtreatment = ischemic_arr & (IVTtime < ivt_time_threshold) & (rng.random(IVTtime.shape) < ivt_probability) & pd.Series(destination_arr.flatten()).str.contains(regex = '[' + config['csc_prefix'] + '|' + config['psc_prefix'] + ']').reshape((num_patients, num_scenarios, num_thresholds))
