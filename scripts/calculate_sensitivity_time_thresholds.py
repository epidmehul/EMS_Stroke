...
def calc_single_map_time_thresholds(map_number):
    ...
    for lvo in (0.141, 0.241, 0.341):
        try:
            ...
            print(map_number)
            retval_df[[(lvo, 'scenario'), (lvo, 'value')]] = pd.DataFrame([[None, None] for i in range(len(col_names))], index = retval_df.index)
            retval_df.drop(lvo, axis = 1, inplace = True)

            retval_df[[(lvo, 'sensitivity'), (lvo, 'threshold')]]= pd.DataFrame([[None, None] for i in range(len(col_names))], index = retval_df.index)
            retval_df.drop((lvo, 'scenario'), axis = 1, inplace = True)
