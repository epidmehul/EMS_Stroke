...
def process_data(filepath = None, plots = True, errorbars = False, additional_file_name = None, config = None, psc_only = False, output_dir = None, intervals = True, interval_width = 0.95, save_format = 'csv', df = None, map_number = None):
    ...
    df['IVTcat'] = pd.cut(df['IVTtreatment'].astype(int) * df['IVTtime'] / 60, bins = np.arange(-0.75, 5, 0.75))
    ...
    ivt_time_props = df.loc[df['ischemic'], ['diagnostic', 'threshold', 'IVTcat']].groupby(['diagnostic', 'threshold', 'IVTcat'], observed = False).size().reset_index().rename({0: 'n'}, axis = 1)
    ...
    if plots:
        ...
        triage_plots.savefig(output_dir / f'map_{str(map_number).zfill(3)}'/ f'{"psc_" if psc_only else ""}{(additional_file_name + "_") if additional_file_name is not None else ""}map_{map_number}_triage_plot.png')

        time_plots.savefig(output_dir / f'map_{str(map_number).zfill(3)}'/ f'{"psc_" if psc_only else ""}{(additional_file_name + "_") if additional_file_name is not None else ""}map_{map_number}_time_plot.png')

        mRS_plots.savefig(output_dir / f'map_{str(map_number).zfill(3)}' / f'{"psc_" if psc_only else ""}{(additional_file_name + "_") if additional_file_name is not None else ""}map_{map_number}_mRS_plot.png')

        prop_plots.savefig(output_dir / f'map_{str(map_number).zfill(3)}' / f'{"psc_" if psc_only else ""}{(additional_file_name + "_") if additional_file_name is not None else ""}map_{map_number}_treatment_proportions_plot.png')

        plt.close()
        plt.style.use('default')
        evt_time_props.iloc[:, 1:].plot(kind = 'bar', stacked = True, figsize = (12, 8))
        plt.axhline(y = evt_time_props.loc[('base', 0), 1:].sum(), color = 'black', ls = '-', lw = 0.75)
        plt.ylabel('Count')
        plt.xlabel('Scenario')
        plt.legend(reverse = True)
        plt.title('EVT times for simulated LVO patients')
        plt.savefig(output_dir / f'map_{str(map_number).zfill(3)}'/ f'{"psc_" if psc_only else ""}{(additional_file_name + "_") if additional_file_name is not None else ""}map_{map_number}_evt_time_props.png')

        plt.close()
        plt.style.use('default')
        ivt_time_props.iloc[:, 1:].plot(kind = 'bar', stacked = True, figsize = (12, 8))
        plt.axhline(y = ivt_time_props.loc[('base', 0), 1:].sum(), color = 'black', ls = '-', lw = 0.75)
        plt.ylabel('Count')
        plt.xlabel('Scenario')
        plt.legend(reverse = True)
        plt.title('IVT times for simulated ischemic patients')
        plt.savefig(output_dir / f'map_{str(map_number).zfill(3)}'/ f'{"psc_" if psc_only else ""}{(additional_file_name + "_") if additional_file_name is not None else ""}map_{map_number}_ivt_time_props.png')
